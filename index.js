...
async function generateStory(btn, resultArea) {
    ...
    const systemPrompt = `你是一个故事创意生成器。请根据以下用户自定义的模板库生成 ${count} 个不同的故事设定。
        ...
        <div class="template-editor-grid">
            <div class="grid-col">
                <div class="template-card">
                    <h4>角色卡模板</h4>
                    <textarea id="edit-char-template" rows="8" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(customTemplates.character)}</textarea>
                ...
                <div class="template-card">
                    <h4>身份词库（留空自定义）</h4>
                    <textarea id="edit-identity" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个身份，例如：&#10;总裁/律师/医生/教师/学生">${escapeHtml(extraTemplates.identity)}</textarea>
                    <button id="save-identity" class="save-tpl-btn">保存身份词库</button>
                </div>
                
                <div class="template-card">
                    <h4>关系词库（留空自定义）</h4>
                    <textarea id="edit-relation" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个关系，例如：&#10;债主与欠债者/上司与下属">${escapeHtml(extraTemplates.relation)}</textarea>
            ...
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>场景词库（留空自定义）</h4>
                    <textarea id="edit-scene" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个场景，例如：&#10;办公室/教室/医院/咖啡厅">${escapeHtml(extraTemplates.scene)}</textarea>
                    <button id="save-scene" class="save-tpl-btn">保存场景词库</button>
                </div>
                
                <div class="template-card">
                    <h4>空间环境玩法（留空自定义）</h4>
                    <textarea id="edit-space" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="户外/室内/公共场所/私密空间等">${escapeHtml(extraTemplates.gameplay_space)}</textarea>
                    <button id="save-space" class="save-tpl-btn">保存空间环境玩法</button>
                </div>
                
                <div class="template-card">
                    <h4>权力动态玩法（留空自定义）</h4>
                    <textarea id="edit-power" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="命令控制/支配服从/奖惩等">${escapeHtml(extraTemplates.gameplay_power)}</textarea>
                    <button id="save-power" class="save-tpl-btn">保存权力动态玩法</button>
                </div>
                
                <div class="template-card">
                    <h4>心理情境玩法（留空自定义）</h4>
                    <textarea id="edit-psychological" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="羞辱/恐惧/期待/依赖等">${escapeHtml(extraTemplates.gameplay_psychological)}</textarea>
                    <button id="save-psychological" class="save-tpl-btn">保存心理情境玩法</button>
                </div>
                
                <div class="template-card">
                    <h4>剧情玩法（留空自定义）</h4>
                    <textarea id="edit-plot" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="意外撞见/临时中断/事后余韵/设局等">${escapeHtml(extraTemplates.gameplay_plot)}</textarea>
...
<div class="button-group"><button id="char-copy" class="copy-btn" disabled>复制</button><button id="char-clear">清空</button></div>
...
<div class="button-group"><button id="user-copy" class="copy-btn" disabled>复制</button><button id="user-clear">清空</button></div>
...
charClear.onclick = () => { charResult.value = ''; charCopy.disabled = true; };
...
charResult.addEventListener('input', () => { const has = !!charResult.value; charCopy.disabled = !has; saveDraft('char', charResult.value); });
...
userClear.onclick = () => { userResult.value = ''; userCopy.disabled = true; };
...
userResult.addEventListener('input', () => { const has = !!userResult.value; userCopy.disabled = !has; saveDraft('user', userResult.value); });
