            ...
            chaptersContainer.innerHTML = currentNovel.chapters.map((ch, idx) => `
                <div class="chapter-item">
                    <div class="chapter-header" data-idx="${idx}">
                        <span>第${idx+1}章：${ch.title || '无标题'}</span>
                        <button class="chapter-toggle">▼</button>
                    </div>
                    <div class="chapter-content" id="chapter-${idx}" style="display: none;">${escapeHtml(ch.content)}</div>
                </div>
            `).join('');
            ...
            document.querySelectorAll('.chapter-header').forEach(header => {
                header.onclick = (e) => {
                    ...
                    if (content.style.display === 'none') {
                        ...
                        header.querySelector('.chapter-toggle').textContent = '▲';
                    } else {
...
async function generateNovel(isContinue = false) {
    ...
    let systemPrompt = novelTemplates.systemPrompt || `你是一个小说创意生成器。根据用户的要求生成小说设定和章节简介。
...
注意：禁止多余形容词，只写干货。`;
    ...
    if (isContinue) {
        ...
        const continuePrompt = novelTemplates.continuePrompt || `你正在续写小说《{title}》。
...
async function generatePlayMix(btn, resultArea) {
    ...
    const systemPrompt = `你是一个创意标签生成器。请生成 ${groupCount} 组人设和玩法组合，每组包含10个人设和10组玩法。
...
7. 不同组之间要有差异，不要重复`;
    ...
    try {
        const content = await callApi(
            [{ role: 'user', content: `请生成 ${groupCount} 组不同的人设和玩法` }],
            systemPrompt,
            { temperature: 0.9, max_tokens: 4000 }
        );
        if (resultArea) {
            resultArea.value = content;
            addToHistory('玩法生成器', `${groupCount}组`, content);
            const copyBtn = resultArea.parentElement?.querySelector('.copy-btn');
            if (copyBtn) copyBtn.disabled = false;
        }
...
async function generateTheater(btn, resultArea) {
    ...
    let systemPrompt = theaterTemplates.systemPrompt || `你是一个小剧场/番外剧情生成器。根据用户的关键词（或随机生成）生成小剧场设定。
    ...
    return `
        <div class="template-editor-grid">
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>小说生成器提示词</h4>
                    <textarea id="edit-novel-system" rows="6" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写小说生成的系统提示词">${escapeHtml(novelTemplates.systemPrompt)}</textarea>
                    <button id="save-novel-system" class="save-tpl-btn">保存小说提示词</button>
                </div>
                
                <div class="template-card">
                    <h4>小剧场生成器提示词</h4>
                    <textarea id="edit-theater-system" rows="6" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写小剧场生成的系统提示词">${escapeHtml(theaterTemplates.systemPrompt)}</textarea>
            ...
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>小说续写提示词</h4>
                    <textarea id="edit-novel-continue" rows="6" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写小说续写的提示词模板">{title}{doneCount}{chapters}{start}{end}{wordCount} 会被自动替换</textarea>
    ...
    if (resetBtn) {
        resetBtn.onclick = () => {
            if (confirm('重置所有模板为默认值？这将丢失所有自定义修改！')) {
                ...
                saveExtraTemplates();
                novelTemplates = { systemPrompt: '', continuePrompt: '' };
                saveNovelTemplates();
                theaterTemplates = { systemPrompt: '' };
                saveTheaterTemplates();
                location.reload();
            }
        };
...
<div class="field"><label>章节列表（点击展开）</label>
...
.dark-mode .chapter-header:hover {
    background: #2a2a2a;
}
