...
let styleGenTemplates = {
    systemPrompt: `你是一位文学风格分析师。根据用户提供的内容（文字片段或关键词），生成详细的文风设定档案。
...
async function generateStyle(btn, resultArea) {
    ...
    if (mode === 'analyze') {
        ...
        userMessage = `请根据以下关键词，生成 ${count} 个不同的文风档案：\n\n${userInput}`;
    }
    ...
    return `
        <div class="template-editor-grid">
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>文风生成提示词</h4>
                    <textarea id="edit-stylegen-system" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(styleGenTemplates.systemPrompt)}</textarea>
...
<textarea id="psychology-input" rows="4" placeholder="例如：一个从小被父母忽视的男孩，长大后总是讨好他人，不敢拒绝..."></textarea></div>
...
<label><input type="radio" name="style-mode" value="keyword"> 从关键词生成</label>
</div>
</div>
<div class="field"><label>输入内容</label>
<textarea id="style-input" rows="5" placeholder="模式A：粘贴一段文字，AI分析其文风&#10;模式B：输入关键词，如“潮湿缱绻的台湾文学 白水烹鲜风格”"></textarea></div>
<div class="field"><label>生成数量</label>
<input type="number" id="style-count" min="1" max="20" value="1">
</div>
<button id="style-gen" class="primary-btn">生成文风</button>
<div class="field"><label>生成结果</label>
<textarea id="style-result" class="result-text" rows="20"></textarea></div>
...
<textarea id="preset-input" rows="5" placeholder="例如：兽类特征加强、凝攻user、上帝视角"></textarea></div>
<div class="field"><label>生成数量</label>
