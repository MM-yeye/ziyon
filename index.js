...
function rebuildTabsAndContents() {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        ...
        btn.onclick = (e) => {
            if (e.target === btn || !e.target.classList?.contains('close-tab')) {
...
// 绑定各种事件（这些事件需要在内容加载后绑定，但内容由rebuildTabsAndContents动态生成，所以事件绑定要放在里面）
// 先加载初始内容
rebuildTabsAndContents();
...
if (ws) {
ws.min = 200;
ws.value = config.panelWidth;
ws.oninput = () => { const v = ws.value; if(widthVal) widthVal.textContent = v; config.panelWidth = parseInt(v); saveConfig(); panel.style.width = `${v}px`; applyTheme(); };
if(widthVal) widthVal.textContent = ws.value;
}
if (hs) {
hs.value = config.panelHeight;
hs.oninput = () => { const v = hs.value; if(heightVal) heightVal.textContent = v; config.panelHeight = parseInt(v); saveConfig(); panel.style.height = `${v}px`; applyTheme(); };
if(heightVal) heightVal.textContent = hs.value;
}
if (ls) {
ls.value = config.panelLeft;
ls.oninput = () => { const v = ls.value; if(leftVal) leftVal.textContent = v; config.panelLeft = parseInt(v); saveConfig(); panel.style.left = `${v}px`; };
if(leftVal) leftVal.textContent = ls.value;
}
if (ts) {
ts.value = config.panelTop;
ts.oninput = () => { const v = ts.value; if(topVal) topVal.textContent = v; config.panelTop = parseInt(v); saveConfig(); panel.style.top = `${v}px`; };
if(topVal) topVal.textContent = ts.value;
}
if (fs) {
fs.value = 13;
fs.oninput = () => { const v = fs.value; if(fontVal) fontVal.textContent = v; panel.style.fontSize = `${v}px`; };
if(fontVal) fontVal.textContent = fs.value;
}
}
...
.tab-bar { display: flex; flex-wrap: wrap; gap: 2px; padding: 8px 12px 0 12px; border-bottom: 1px solid #e0e0e0; background: #fafafa; }
...
.dark-mode .char-item:hover,
.dark-mode .history-item:hover {
    background: #2a2a2a;
}
.dark-mode .quick-theme-btn {
    background: #3a3a3a;
    color: #e0e0e0;
}
.dark-mode .quick-theme-btn:hover {
    background: #4a4a4a;
}
.dark-mode .size-control-btn {
    background: #3a3a3a;
    color: #e0e0e0;
}
.dark-mode .size-control-btn:hover {
    background: #4a4a4a;
}
