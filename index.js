        ...
        <div class="custom-page-editor">
            <div class="custom-page-form">
                ...
                <div class="button-group">
                    ...
                    <button id="reset-default-pages" class="menu_button">恢复默认</button>
                </div>
            </div>
...
function resetToDefaultPages() {
    if (confirm('恢复默认会将所有自定义页面清空，确定吗？')) {
        customPages = [];
        saveCustomPages();
        refreshCustomPagesUI();
        rebuildTabs();
...
function bindCustomPageManagerEvents() {
    ...
    const resetBtn = document.getElementById('reset-default-pages');
    if (resetBtn) {
        resetBtn.onclick = () => resetToDefaultPages();
    }
}
...
function rebuildTabs() {
    ...
    const originalOrder = ['api', 'char', 'batch', 'user', 'world', 'world-extend', 'relation', 'wardrobe', 'story', 'playmix', 'novel', 'theater', 'psychology', 'stylegen', 'presetgen', 'custom-mgr', 'history', 'templates', 'template-edit', 'theme', 'size'];
...
wardrobeGen.onclick = async () => { 
    ...
    let subMode = null;
    if (mode === 'qq') {
        subMode = document.querySelector('input[name="qq-submode"]:checked')?.value || 'freeform';
    }
...
})();.trim(); if (!n) { toastr.warning('请输入模板名称'); return; } if (!wardrobeResult.value) { toastr.warning('没有内容'); return; } config.savedTemplates.push({ name: n, content: wardrobeResult.value }); saveConfig(); refreshTemplateList(); toastr.success(`已保存: ${n}`); wardrobeTplName.value
