...
function getDeletePageButtonHtml(pageId, pageName) {
    return `<div class="delete-page-container" style="margin-top: 16px; text-align: center; padding: 12px; border-top: 1px solid #e0e0e0;">
        <button id="delete_page_${pageId}" class="delete-page-btn" style="background: #d32f2f; color: white; padding: 6px 16px;">🗑️ 删除此页面</button>
    </div>`;
}
...
function bindCustomPageEvents(pageId) {
    ...
    genBtn.onclick = async () => {
        ...
        if (!template || !template.trim()) {
            if (typeof toastr !== 'undefined') toastr.warning(`请先在模板编辑页填写【自定义-${page.name}-${selectedSubmode}】提示词`);
            return;
        }
...
function addBuiltinPageDeleteButton(tabId, pageName) {
    ...
    // 检查是否已经有删除按钮区域
    if (container.querySelector('.builtin-delete-container')) return '';
    
    const deleteHtml = `<div class="builtin-delete-container" style="margin-top: 16px; padding: 12px; border-top: 1px solid #e0e0e0; text-align: center;">
        <button id="delete_builtin_${tabId}" class="delete-page-btn" style="background: #d32f2f; color: white; padding: 6px 16px;">🗑️ 删除此页面</button>
    </div>`;
    ...
    for (const tabId of activeTabs) {
        if (builtInTabs[tabId]) {
            ...
            if (tabId !== 'api' && tabId !== 'custom-mgr') {
                setTimeout(() => {
                    addBuiltinPageDeleteButton(tabId, builtInTabs[tabId]);
                }, 50);
            }
        }
    }
...
function bindAllGenerationEvents() {
    ...
    if (charGen && charInput && charResult) {
        charGen.onclick = () => generateCharacter(charInput.value, 'character', charGen, charResult);
    }
    ...
    if (charSaveTpl && charTemplateName && charResult) {
        charSaveTpl.onclick = () => {
            ...
            if (name && charResult.value) {
                ...
                if (typeof toastr !== 'undefined') toastr.warning('请输入模板名称且结果不为空');
            }
        };
    }
    ...
    if (userGen && userInput && userResult) {
        userGen.onclick = () => generateCharacter(userInput.value, 'user', userGen, userResult);
    }
    ...
    if (batchGen && batchInput && batchResult) {
        batchGen.onclick = () => batchGenerateCharacters(batchInput.value, batchGen, batchResult);
    }
    ...
    if (worldGen && worldInput && worldResult) {
        worldGen.onclick = () => generateWorldbook(worldInput.value, worldGen, worldResult);
    }
    ...
    if (worldExtendGen) {
        worldExtendGen.onclick = () => generateWorldExtend(worldExtendGen);
    }
    ...
    if (relationAdd && relationName) {
        relationAdd.onclick = () => addRelationChar(relationName.value, relationDesc?.value || '');
    }
    const relationGen = document.getElementById('relation-gen');
    const relationResult = document.getElementById('relation-result');
    if (relationGen && relationResult) {
        relationGen.onclick = () => generateRelationText(relationGen, relationResult);
    }
    ...
    if (wardrobeModeRadios.length) {
        wardrobeModeRadios.forEach(r => {
            r.onchange = () => {
                if (r.checked) wardrobeMode = r.value;
                const qqOptions = document.getElementById('qq-mode-options');
                if (qqOptions) qqOptions.style.display = wardrobeMode === 'qq' ? 'block' : 'none';
            };
        });
    }
    ...
    if (storyGen && storyResult) {
        storyGen.onclick = () => generateStory(storyGen, storyResult);
    }
    ...
    if (playmixGen && playmixResult) {
        playmixGen.onclick = () => generatePlayMix(playmixGen, playmixResult);
    }
    ...
    if (novelWordCount && wordCountVal) {
        novelWordCount.oninput = () => { wordCountVal.textContent = novelWordCount.value; };
    }
    
    // 小剧场
    const theaterGen = document.getElementById('theater-gen');
    const theaterResult = document.getElementById('theater-result');
    if (theaterGen && theaterResult) {
        theaterGen.onclick = () => generateTheater(theaterGen, theaterResult);
    }
    ...
    if (psychologyAnalyze && psychologyResult) {
        psychologyAnalyze.onclick = () => generatePsychologyAnalysis(psychologyAnalyze, psychologyResult);
    }
    ...
    if (mbtiTest && mbtiResult) {
        mbtiTest.onclick = () => generateMBTITest(mbtiTest, mbtiResult);
    }
    ...
    if (styleGen && styleResult) {
        styleGen.onclick = () => generateStyle(styleGen, styleResult);
    }
    ...
    if (presetGen && presetResult) {
        presetGen.onclick = () => generatePreset(presetGen, presetResult);
    }
    ...
    if (quickThemeBtn) {
        quickThemeBtn.onclick = () => {
            ...
            saveConfig();
            applyTheme();
            const lightRadio = document.querySelector('input[name="theme-mode"][value="light"]');
            const darkRadio = document.querySelector('input[name="theme-mode"][value="dark"]');
            if (lightRadio && darkRadio) {
                if (config.themeMode === 'light') lightRadio.checked = true;
...
function bindApiConfigEvents() {
    ...
    if (fetchBtn) {
        fetchBtn.onclick = () => fetchModelList();
    }
    
    const useLocalBtn = document.getElementById('apg-use-local-api');
    if (useLocalBtn) {
        useLocalBtn.onclick = () => useLocalApiConfig();
    }
}
    ...
    return `
        <div class="template-editor-grid">
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>身份词库</h4>
                    <textarea id="edit-identity" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个身份">${escapeHtml(extraTemplates.identity || '')}</textarea>
                    <button id="save-identity" class="save-tpl-btn">保存身份词库</button>
                </div>
                
                <div class="template-card">
                    <h4>关系词库</h4>
                    <textarea id="edit-relation" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个关系">${escapeHtml(extraTemplates.relation || '')}</textarea>
            ...
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>场景词库</h4>
                    <textarea id="edit-scene" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个场景">${escapeHtml(extraTemplates.scene || '')}</textarea>
                    <button id="save-scene" class="save-tpl-btn">保存场景词库</button>
                </div>
                
                <div class="template-card">
                    <h4>空间环境玩法</h4>
                    <textarea id="edit-space" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个玩法">${escapeHtml(extraTemplates.gameplay_space || '')}</textarea>
                    <button id="save-space" class="save-tpl-btn">保存空间环境玩法</button>
                </div>
                
                <div class="template-card">
                    <h4>权力动态玩法</h4>
                    <textarea id="edit-power" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个玩法">${escapeHtml(extraTemplates.gameplay_power || '')}</textarea>
                    <button id="save-power" class="save-tpl-btn">保存权力动态玩法</button>
                </div>
                
                <div class="template-card">
                    <h4>心理情境玩法</h4>
                    <textarea id="edit-psychological" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个玩法">${escapeHtml(extraTemplates.gameplay_psychological || '')}</textarea>
                    <button id="save-psychological" class="save-tpl-btn">保存心理情境玩法</button>
                </div>
                
                <div class="template-card">
                    <h4>剧情玩法</h4>
                    <textarea id="edit-plot" rows="4" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="每行一个玩法">${escapeHtml(extraTemplates.gameplay_plot || '')}</textarea>
...
function bindTemplateEditorEvents() {
    ...
    if (saveChar) {
        saveChar.onclick = () => {
            ...
            if (val !== undefined) {
                ...
                saveCustomTemplates();
                if (typeof toastr !== 'undefined') toastr.success('角色卡模板已保存');
            }
        };
    }
    ...
    if (saveRelation) {
        saveRelation.onclick = () => {
            ...
            if (val !== undefined) {
                ...
                saveCustomTemplates();
                if (typeof toastr !== 'undefined') toastr.success('关系描述模板已保存');
            }
        };
    }
    ...
    if (saveWardrobe) {
        saveWardrobe.onclick = () => {
            ...
            if (val !== undefined) {
                ...
                saveCustomTemplates();
                if (typeof toastr !== 'undefined') toastr.success('魔法衣橱模板已保存');
            }
        };
    }
    ...
    if (saveWorldbook) {
        saveWorldbook.onclick = () => {
            ...
            if (val !== undefined) {
                ...
                saveCustomTemplates();
                if (typeof toastr !== 'undefined') toastr.success('世界书提示词已保存');
            }
        };
    }
    ...
    if (saveExtend) {
        saveExtend.onclick = () => {
            ...
            saveCustomTemplates();
            if (typeof toastr !== 'undefined') toastr.success('扩展模板已保存');
        };
    }
    ...
    if (saveIdentity) {
        saveIdentity.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('身份词库已保存');
        };
    }
    ...
    if (saveRelationExtra) {
        saveRelationExtra.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('关系词库已保存');
        };
    }
    ...
    if (saveScene) {
        saveScene.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('场景词库已保存');
        };
    }
    ...
    if (saveSpace) {
        saveSpace.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('空间环境玩法已保存');
        };
    }
    ...
    if (savePower) {
        savePower.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('权力动态玩法已保存');
        };
    }
    ...
    if (savePsychological) {
        savePsychological.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('心理情境玩法已保存');
        };
    }
    ...
    if (savePlot) {
        savePlot.onclick = () => {
            ...
            saveExtraTemplates();
            if (typeof toastr !== 'undefined') toastr.success('剧情玩法已保存');
        };
    }
    ...
    if (saveNovelSystem) {
        saveNovelSystem.onclick = () => {
            ...
            saveNovelTemplates();
            if (typeof toastr !== 'undefined') toastr.success('小说提示词已保存');
        };
    }
    ...
    if (saveNovelContinue) {
        saveNovelContinue.onclick = () => {
            ...
            saveNovelTemplates();
            if (typeof toastr !== 'undefined') toastr.success('续写提示词已保存');
        };
    }
    ...
    if (saveTheaterSystem) {
        saveTheaterSystem.onclick = () => {
            ...
            if (typeof toastr !== 'undefined') toastr.success('小剧场提示词已保存');
        };
    }
    ...
    if (savePsychologyAnalysis) {
        savePsychologyAnalysis.onclick = () => {
            ...
            if (typeof toastr !== 'undefined') toastr.success('心理分析提示词已保存');
        };
    }
    ...
    if (saveMbtiPrompt) {
        saveMbtiPrompt.onclick = () => {
            ...
            savePsychologyTemplates();
            if (typeof toastr !== 'undefined') toastr.success('MBTI分析提示词已保存');
        };
    }
    ...
    if (saveStyleGenSystem) {
        saveStyleGenSystem.onclick = () => {
            ...
            if (typeof toastr !== 'undefined') toastr.success('文风生成提示词已保存');
        };
    }
    ...
    if (savePresetGenSystem) {
        savePresetGenSystem.onclick = () => {
            ...
            if (typeof toastr !== 'undefined') toastr.success('预设生成提示词已保存');
        };
    }
    ...
    if (saveQQFreeform) {
        saveQQFreeform.onclick = () => {
            ...
            if (typeof toastr !== 'undefined') toastr.success('QQ服装自由描述模板已保存');
        };
    }
            ...
            if (saveBtn) {
                saveBtn.onclick = () => {
                    ...
                    if (val !== undefined) {
                        ...
                        if (typeof toastr !== 'undefined') toastr.success(`【自定义-${page.name}-${submode.name}】提示词已保存`);
                    }
                };
            }
        }
...
function applyTheme() {
    ...
    if (config.themeMode === 'light') {
        ...
        // 全局样式覆盖
        const styleOverride = document.getElementById('ai-gen-theme-style');
        if (styleOverride) styleOverride.remove();
        const newStyle = document.createElement('style');
        newStyle.id = 'ai-gen-theme-style';
        newStyle.textContent = `
            .ai-panel.light-mode .field input, 
            .ai-panel.light-mode .field textarea, 
            .ai-panel.light-mode .field select {
                background: #fff !important;
                border-color: #ddd !important;
                color: #333 !important;
            }
            .ai-panel.light-mode .result-text {
                background: #fafafa !important;
                color: #333 !important;
            }
            .ai-panel.light-mode .tab-bar {
                background: #fafafa !important;
            }
            .ai-panel.light-mode .tab-btn.active {
                background: #fff !important;
                color: #1a1a1a !important;
            }
            .ai-panel.light-mode .panel-header {
                background: #f8f8f8 !important;
            }
            .ai-panel.light-mode .template-card,
            .ai-panel.light-mode .custom-page-form,
            .ai-panel.light-mode .custom-page-list {
        ...
        newStyle.id = 'ai-gen-theme-style';
        newStyle.textContent = `
            .ai-panel.dark-mode .field input, 
            .ai-panel.dark-mode .field textarea, 
            .ai-panel.dark-mode .field select {
                background: #2d2d2d !important;
                border-color: #444 !important;
                color: #e0e0e0 !important;
            }
            .ai-panel.dark-mode .result-text {
                background: #252525 !important;
                color: #e0e0e0 !important;
            }
            .ai-panel.dark-mode .tab-bar {
                background: #252525 !important;
                border-bottom-color: #333 !important;
            }
            .ai-panel.dark-mode .tab-btn {
                color: #aaa !important;
            }
            .ai-panel.dark-mode .tab-btn.active {
            ...
            .ai-panel.dark-mode .template-card,
            .ai-panel.dark-mode .custom-page-form,
            .ai-panel.dark-mode .custom-page-list {
                background: #2a2a2a !important;
                border-color: #444 !important;
            }
            .ai-panel.dark-mode .char-item,
            .ai-panel.dark-mode .history-item,
            .ai-panel.dark-mode .template-item {
                border-bottom-color: #333 !important;
            }
            .ai-panel.dark-mode .char-item:hover,
            .ai-panel.dark-mode .history-item:hover {
                background: #2a2a2a !important;
            }
            .ai-panel.dark-mode .quick-theme-btn,
            .ai-panel.dark-mode .size-control-btn {
                background: #3a3a3a !important;
                color: #e0e0e0 !important;
            }
            .ai-panel.dark-mode .primary-btn {
                background: #0d6efd !important;
            }
            .ai-panel.dark-mode .copy-btn,
            .ai-panel.dark-mode .save-tpl-btn {
                background: #0d6efd !important;
            }
            .ai-panel.dark-mode .chapter-card {
                border-color: #444 !important;
            }
            .ai-panel.dark-mode .chapter-title {
                background: #2d2d2d !important;
                border-bottom-color: #444 !important;
            }
            .ai-panel.dark-mode .chapter-content-textarea {
                background: #252525 !important;
                color: #e0e0e0 !important;
            }
            .ai-panel.dark-mode .novel-title-display {
                background: #2d2d2d !important;
                color: #e0e0e0 !important;
            }
        `;
...
if (typeof toastr !== 'undefined') toastr.success(`宽度: ${newWidth}px`);
};
}
...
if (typeof toastr !== 'undefined') toastr.success('面板已重置到右侧');
};
}
...
.tab-btn { padding: 6px 12px; background: none; border: none; cursor: pointer; font-size: 12px; color: #666; border-radius: 8px 8px 0 0; position: relative; }
.tab-btn.active { font-weight: 500; border: 1px solid #e0e0e0; border-bottom-color: #fff; margin-bottom: -1px; background: #fff; color: #1a1a1a; }
...
.field input, .field textarea, .field select { width: 100%; padding: 8px 10px; border: 1px solid #ddd; border-radius: 8px; font-size: 13px; box-sizing: border-box; background: #fff; }
...
.delete-page-btn { padding: 6px 16px; border-radius: 8px; cursor: pointer; border: none; }
.delete-page-btn:hover { opacity: 0.85; }
.builtin-delete-container { margin-top: 16px; padding: 12px; border-top: 1px solid #e0e0e0; text-align: center; }
...
.custom-page-form { border: 1px solid #ddd; border-radius: 8px; padding: 16px; }
