...
function resetCustomPagesToDefault() {
    if (confirm('恢复默认将删除所有自定义页面，此操作不可撤销。确定吗？')) {
        customPages = [];
        saveCustomPages();
        refreshCustomPagesUI();
        rebuildTabs();
        refreshTemplateEditor();
        toastr.success('已恢复默认，所有自定义页面已清空');
    }
}
        ...
        <div class="custom-page-editor">
            ...
            <div class="custom-page-list">
                ...
                <div class="button-group" style="margin-top: 12px;">
                    <button id="reset-custom-pages" class="reset-btn" style="background: #d32f2f;">恢复默认（清空所有）</button>
                </div>
            </div>
        </div>
    `;
...
function refreshCustomPagesUI() {
    ...
    const resetBtn = document.getElementById('reset-custom-pages');
    if (resetBtn) {
        resetBtn.onclick = () => resetCustomPagesToDefault();
    }
}
