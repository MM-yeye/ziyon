...
try {
const nameMatch = yamlContent.match(/Chinese name:\s*(.+)/m) || yamlContent.match(/chinese_name:\s*(.+)/m);
const name = nameMatch ? nameMatch[1].trim() : '新角色';
const desc = extractField(yamlContent, 'appearance') || extractField(yamlContent, '外貌特征');
const personality = extractField(yamlContent, 'personality') || extractField(yamlContent, '性格');
const scenario = extractField(yamlContent, 'background_story') || extractField(yamlContent, '背景故事');
const first_mes = `你好，我是${name}`;
const mes_example = '';
...
<h4 style="margin: 0 0 12px 0;">角色卡模板</h4>
...
<button id="save-char-template" class="menu_button" style="margin: 8px 0;">保存角色卡模板</button>

<h4 style="margin: 16px 0 12px 0;">关系描述模板</h4>
<textarea id="edit-relation-template" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(customTemplates.relationship)}</textarea>
<button id="save-relation-template" class="menu_button" style="margin: 8px 0;">保存关系描述模板</button>

<h4 style="margin: 16px 0 12px 0;">魔法衣橱模板</h4>
<textarea id="edit-wardrobe-template" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(customTemplates.wardrobe)}</textarea>
<button id="save-wardrobe-template" class="menu_button" style="margin: 8px 0;">保存魔法衣橱模板</button>

<h4 style="margin: 16px 0 12px 0;">世界书提示词</h4>
...
<button id="save-worldbook-prompt" class="menu_button" style="margin: 8px 0;">保存世界书提示词</button>

<h4 style="margin: 16px 0 12px 0;">世界书扩展模板</h4>
${Object.entries(DEFAULT_WORLD_EXTEND_TEMPLATES).map(([key, template]) => `
<div style="margin-bottom: 12px;">
<label><strong>${typeNames[key] || key}</strong></label>
<textarea id="edit-extend-${key}" rows="6" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(customTemplates.worldExtend[key] || template)}</textarea>
</div>
`).join('')}
<button id="save-extend-templates" class="menu_button" style="margin: 8px 0;">保存所有扩展模板</button>

<hr style="margin: 16px 0;">
<div class="button-group">
<button id="reset-all-templates" style="background: #d32f2f; color: white;">重置所有模板为默认</button>
</div>
</div>
`;
}
...
function loadStoryTemplates() {
    try {
        const saved = localStorage.getItem(`${EXTENSION_NAME}_story_templates`);
        if (saved) {
            storyTemplates = { ...storyTemplates, ...JSON.parse(saved) };
        }
    } catch(e) {}
}

function saveStoryTemplates() {
    try {
        localStorage.setItem(`${EXTENSION_NAME}_story_templates`, JSON.stringify(storyTemplates));
    } catch(e) {}
}
...
function renderStoryEditor() {
    return `
        <div class="story-editor">
            <h4 style="margin:0 0 8px 0;">身份模板</h4>
            <textarea id="story-identity-tpl" rows="4" style="width:100%; font-family:monospace; font-size:12px;" placeholder="示例：&#10;总裁/律师/医生/教师/学生/警察/黑客/管家/女仆/偶像">${escapeHtml(storyTemplates.identity)}</textarea>
            <h4 style="margin:16px 0 8px 0;">关系模板</h4>
            <textarea id="story-relation-tpl" rows="4" style="width:100%; font-family:monospace; font-size:12px;" placeholder="示例：&#10;债主与欠债者/上司与下属/医生与病人/老师与学生/主人与仆人">${escapeHtml(storyTemplates.relation)}</textarea>
            <h4 style="margin:16px 0 8px 0;">场景模板</h4>
            <textarea id="story-scene-tpl" rows="4" style="width:100%; font-family:monospace; font-size:12px;" placeholder="示例：&#10;办公室/教室/医院/咖啡厅/私家车/地铁/电梯/天台/酒店房间">${escapeHtml(storyTemplates.scene)}</textarea>
            <h4 style="margin:16px 0 8px 0;">玩法模板</h4>
            <textarea id="story-gameplay-tpl" rows="4" style="width:100%; font-family:monospace; font-size:12px;" placeholder="示例：&#10;命令与服从/言语控制/时间限制/公共场所风险/轻束缚">${escapeHtml(storyTemplates.gameplay)}</textarea>
            <button id="save-story-tpls" class="menu_button" style="margin:12px 0;">保存所有故事模板</button>
        </div>
    `;
}

function bindStoryEditorEvents() {
    const saveBtn = document.getElementById('save-story-tpls');
    if (saveBtn) {
        saveBtn.onclick = () => {
            storyTemplates.identity = document.getElementById('story-identity-tpl')?.value || '';
            storyTemplates.relation = document.getElementById('story-relation-tpl')?.value || '';
            storyTemplates.scene = document.getElementById('story-scene-tpl')?.value || '';
            storyTemplates.gameplay = document.getElementById('story-gameplay-tpl')?.value || '';
            saveStoryTemplates();
            toastr.success('故事模板已保存');
        };
    }
}
...
async function generateStory(btn, resultArea) {
    ...
    const count = parseInt(document.getElementById('story-count')?.value || '1');
    if (isNaN(count) || count < 1 || count > 10) {
        toastr.warning('数量请在1-10之间');
        return;
    }
    
    const systemPrompt = `你是一个故事创意生成器。请根据以下模板生成 ${count} 个不同的故事设定。

模板内容（用户可编辑，当前可能为空，请根据常识补充）：
身份库：${storyTemplates.identity || '总裁/律师/医生/教师/学生/警察/黑客/管家/女仆/偶像'}
关系库：${storyTemplates.relation || '债主与欠债者/上司与下属/医生与病人/老师与学生/主人与仆人'}
场景库：${storyTemplates.scene || '办公室/教室/医院/咖啡厅/私家车/地铁/电梯/天台/酒店房间'}
玩法库：${storyTemplates.gameplay || '命令与服从/言语控制/时间限制/公共场所风险/轻束缚'}

生成规则：
1. 每个故事包含：身份A、身份B、关系性质、场景、核心玩法
2. 输出格式（严格遵守）：
---
身份A：xxx
身份B：xxx
...
玩法：xxx
---
3. 禁止添加外貌描写、心理描写、环境渲染
4. 禁止使用"美丽""漂亮""性感"等形容词
5. 每个故事独立，用 "---" 分隔
6. 如果模板内容不足，请根据常识合理补充`;

    if (btn) { btn.disabled = true; btn.textContent = '生成中...'; }
    try {
        const content = await callApi(
            [{ role: 'user', content: `请生成 ${count} 个不同的故事设定` }],
            systemPrompt,
            { temperature: 0.85, max_tokens: 3000 }
        );
        if (resultArea) {
            resultArea.value = content;
            addToHistory('故事生成器', `${count}个故事`, content);
            const copyBtn = resultArea.parentElement?.querySelector('.copy-btn');
            if (copyBtn) copyBtn.disabled = false;
        }
...
async function generatePlayMix(btn, resultArea) {
    ...
    const systemPrompt = `你是一个创意标签生成器。请生成10组人设和10组玩法组合。
...
7. 可以自由发挥，不限于常见组合`;
    ...
    try {
        const content = await callApi(
            [{ role: 'user', content: '请生成10组人设和10组玩法' }],
            systemPrompt,
            { temperature: 0.9, max_tokens: 2000 }
        );
        if (resultArea) {
            resultArea.value = content;
            addToHistory('玩法生成器', '10人设+10玩法', content);
            const copyBtn = resultArea.parentElement?.querySelector('.copy-btn');
            if (copyBtn) copyBtn.disabled = false;
        }
        toastr.success('生成成功');
...
function loadXiaoxiaoleTemplates() {
    try {
        const saved = localStorage.getItem(`${EXTENSION_NAME}_xiaoxiaole_templates`);
        if (saved) {
            xiaoxiaoleTemplates = { ...xiaoxiaoleTemplates, ...JSON.parse(saved) };
        }
    } catch(e) {}
}

function saveXiaoxiaoleTemplates() {
    try {
        localStorage.setItem(`${EXTENSION_NAME}_xiaoxiaole_templates`, JSON.stringify(xiaoxiaoleTemplates));
    } catch(e) {}
}

function renderXiaoxiaoleEditor() {
    return `
        <div class="xiaoxiaole-editor">
            <h4 style="margin:0 0 8px 0;">身份模板</h4>
            <textarea id="xxl-identity" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="在这里填写身份词库，每行一个或逗号分隔">${escapeHtml(xiaoxiaoleTemplates.identity)}</textarea>
            
            <h4 style="margin:16px 0 8px 0;">关系模板</h4>
            <textarea id="xxl-relation" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="在这里填写关系词库">${escapeHtml(xiaoxiaoleTemplates.relation)}</textarea>
            
            <h4 style="margin:16px 0 8px 0;">场景模板</h4>
            <textarea id="xxl-scene" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="在这里填写场景词库">${escapeHtml(xiaoxiaoleTemplates.scene)}</textarea>
            
            <h4 style="margin:16px 0 8px 0;">空间环境玩法</h4>
            <textarea id="xxl-space" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="户外/室内/公共场所/私密空间等">${escapeHtml(xiaoxiaoleTemplates.gameplay_space)}</textarea>
            
            <h4 style="margin:16px 0 8px 0;">权力动态玩法</h4>
            <textarea id="xxl-power" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="命令控制/支配服从/奖惩等">${escapeHtml(xiaoxiaoleTemplates.gameplay_power)}</textarea>
            
            <h4 style="margin:16px 0 8px 0;">心理情境玩法</h4>
            <textarea id="xxl-psychological" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="羞辱/恐惧/期待/依赖等">${escapeHtml(xiaoxiaoleTemplates.gameplay_psychological)}</textarea>
            
            <h4 style="margin:16px 0 8px 0;">剧情玩法</h4>
            <textarea id="xxl-plot" rows="6" style="width:100%; font-family:monospace; font-size:12px;" placeholder="意外撞见/临时中断/事后余韵/设局等">${escapeHtml(xiaoxiaoleTemplates.gameplay_plot)}</textarea>
            
            <button id="save-xxl-tpls" class="menu_button" style="margin:12px 0;">保存所有消消乐模板</button>
            <hr>
            <div class="field">
                <label>生成数量</label>
                <input type="number" id="xxl-count" min="1" max="20" value="5">
            </div>
            <button id="generate-from-xxl" class="primary-btn">从模板生成内容</button>
            <div class="field"><label>生成结果</label><textarea id="xxl-result" class="result-text" rows="12"></textarea></div>
            <div class="button-group"><button id="xxl-copy" class="copy-btn" disabled>复制</button><button id="xxl-clear">清空</button></div>
        </div>
    `;
}

function bindXiaoxiaoleEvents() {
    const saveBtn = document.getElementById('save-xxl-tpls');
    if (saveBtn) {
        saveBtn.onclick = () => {
            xiaoxiaoleTemplates.identity = document.getElementById('xxl-identity')?.value || '';
            xiaoxiaoleTemplates.relation = document.getElementById('xxl-relation')?.value || '';
            xiaoxiaoleTemplates.scene = document.getElementById('xxl-scene')?.value || '';
            xiaoxiaoleTemplates.gameplay_space = document.getElementById('xxl-space')?.value || '';
            xiaoxiaoleTemplates.gameplay_power = document.getElementById('xxl-power')?.value || '';
            xiaoxiaoleTemplates.gameplay_psychological = document.getElementById('xxl-psychological')?.value || '';
            xiaoxiaoleTemplates.gameplay_plot = document.getElementById('xxl-plot')?.value || '';
            saveXiaoxiaoleTemplates();
            toastr.success('消消乐模板已保存');
        };
    }
    
    const genBtn = document.getElementById('generate-from-xxl');
    const resultArea = document.getElementById('xxl-result');
    const copyBtn = document.getElementById('xxl-copy');
    const clearBtn = document.getElementById('xxl-clear');
    ...
    if (genBtn) {
        genBtn.onclick = async () => {
            ...
            const count = parseInt(document.getElementById('xxl-count')?.value || '5');
            const systemPrompt = `你是一个创意生成器。请根据以下用户自定义的模板库，生成 ${count} 个创意组合。

身份库：${xiaoxiaoleTemplates.identity || '（用户未填写，请根据常识补充常见身份）'}
关系库：${xiaoxiaoleTemplates.relation || '（用户未填写，请根据常识补充常见关系）'}
场景库：${xiaoxiaoleTemplates.scene || '（用户未填写，请根据常识补充常见场景）'}
空间环境玩法：${xiaoxiaoleTemplates.gameplay_space || '（用户未填写）'}
权力动态玩法：${xiaoxiaoleTemplates.gameplay_power || '（用户未填写）'}
心理情境玩法：${xiaoxiaoleTemplates.gameplay_psychological || '（用户未填写）'}
剧情玩法：${xiaoxiaoleTemplates.gameplay_plot || '（用户未填写）'}

生成规则：
1. 每个组合包含：身份A、身份B、关系、场景、玩法组合（3-6个玩法标签）
2. 输出格式：
【组合1】
身份A：xxx
身份B：xxx
...
---
3. 禁止外貌描写、禁止形容词
4. 玩法标签从用户提供的模板中选取或合理组合
5. 如果用户模板为空，请根据常识补充`;

            if (genBtn) { genBtn.disabled = true; genBtn.textContent = '生成中...'; }
            try {
                const content = await callApi(
                    [{ role: 'user', content: `请生成 ${count} 个创意组合` }],
                    systemPrompt,
                ...
                if (resultArea) {
                    resultArea.value = content;
                    if (copyBtn) copyBtn.disabled = false;
                    addToHistory('消消乐', `${count}个组合`, content);
                }
                toastr.success(`生成 ${count} 个组合成功`);
            } catch(err) {
                toastr.error(`失败: ${err.message}`);
            } finally {
                if (genBtn) { genBtn.disabled = false; genBtn.textContent = '从模板生成内容'; }
            }
        };
    }
...
const modelOpts = (apiConfig.availableModels || [])
.map(m => `<option value="${escapeHtml(m)}" ${m === apiConfig.apiModel ? 'selected' : ''}>${escapeHtml(m)}</option>`).join('');
...
<button class="tab-btn" data-tab="xiaoxiaole">消消乐</button>
...
<div class="field"><label>生成数量</label><input type="number" id="story-count" min="1" max="10" value="3"></div>
...
<div class="button-group"><button id="story-copy" class="copy-btn" disabled>复制</button><button id="story-clear">清空</button></div>
<div class="field"><label>编辑模板（可自定义身份/关系/场景/玩法库）</label></div>
${renderStoryEditor()}
</div>
...
<div class="button-group"><button id="playmix-copy" class="copy-btn" disabled>复制</button><button id="playmix-clear">清空</button></div>
</div>

<!-- 消消乐 -->
<div class="tab-content" id="tab-xiaoxiaole">
${renderXiaoxiaoleEditor()}
</div>
...
if (storyClear) storyClear.onclick = () => { if(storyResult) storyResult.value = ''; if(storyCopy) storyCopy.disabled = true; };
if(storyResult) storyResult.addEventListener('input', () => { if(storyCopy) storyCopy.disabled = !storyResult.value; saveDraft('story', storyResult.value); });
...
if (playmixClear) playmixClear.onclick = () => { if(playmixResult) playmixResult.value = ''; if(playmixCopy) playmixCopy.disabled = true; };
if(playmixResult) playmixResult.addEventListener('input', () => { if(playmixCopy) playmixCopy.disabled = !playmixResult.value; saveDraft('playmix', playmixResult.value); });
if(playmixResult) playmixResult.value = loadDraft('playmix');

// 消消乐结果
const xxlResult = document.getElementById('xxl-result');
const xxlCopy = document.getElementById('xxl-copy');
const xxlClear = document.getElementById('xxl-clear');
if(xxlResult) xxlResult.addEventListener('input', () => { if(xxlCopy) xxlCopy.disabled = !xxlResult.value; saveDraft('xiaoxiaole', xxlResult.value); });
if(xxlResult) xxlResult.value = loadDraft('xiaoxiaole');
...
// 加载故事模板和消消乐模板
loadStoryTemplates();
loadXiaoxiaoleTemplates();
// 如果有保存的内容，填充到编辑框
setTimeout(() => {
    const identityEl = document.getElementById('story-identity-tpl');
    if(identityEl && storyTemplates.identity) identityEl.value = storyTemplates.identity;
    const relationEl = document.getElementById('story-relation-tpl');
    if(relationEl && storyTemplates.relation) relationEl.value = storyTemplates.relation;
    const sceneEl = document.getElementById('story-scene-tpl');
    if(sceneEl && storyTemplates.scene) sceneEl.value = storyTemplates.scene;
    const gameplayEl = document.getElementById('story-gameplay-tpl');
    if(gameplayEl && storyTemplates.gameplay) gameplayEl.value = storyTemplates.gameplay;
    
    const xxlIdentity = document.getElementById('xxl-identity');
    if(xxlIdentity && xiaoxiaoleTemplates.identity) xxlIdentity.value = xiaoxiaoleTemplates.identity;
    const xxlRelation = document.getElementById('xxl-relation');
    if(xxlRelation && xiaoxiaoleTemplates.relation) xxlRelation.value = xiaoxiaoleTemplates.relation;
    const xxlScene = document.getElementById('xxl-scene');
    if(xxlScene && xiaoxiaoleTemplates.scene) xxlScene.value = xiaoxiaoleTemplates.scene;
    const xxlSpace = document.getElementById('xxl-space');
    if(xxlSpace && xiaoxiaoleTemplates.gameplay_space) xxlSpace.value = xiaoxiaoleTemplates.gameplay_space;
    const xxlPower = document.getElementById('xxl-power');
    if(xxlPower && xiaoxiaoleTemplates.gameplay_power) xxlPower.value = xiaoxiaoleTemplates.gameplay_power;
    const xxlPsy = document.getElementById('xxl-psychological');
    if(xxlPsy && xiaoxiaoleTemplates.gameplay_psychological) xxlPsy.value = xiaoxiaoleTemplates.gameplay_psychological;
    const xxlPlot = document.getElementById('xxl-plot');
    if(xxlPlot && xiaoxiaoleTemplates.gameplay_plot) xxlPlot.value = xiaoxiaoleTemplates.gameplay_plot;
}, 100);
}
...
.template-editor textarea, .story-editor textarea, .xiaoxiaole-editor textarea { font-family: monospace; font-size: 12px; line-height: 1.4; }
