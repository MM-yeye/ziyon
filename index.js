...
// ========== 魔法衣橱 ==========
async function generateWardrobe(userInput, mode, btn, resultArea) {
    if (!apiConfig.apiKey || !apiConfig.apiUrl || !apiConfig.apiModel) {
        toastr.error('请先配置 API（地址、Key、模型）');
        return null;
    }
...
${customTemplates.wardrobe}`;
    ...
    try {
        ...
        if (resultArea) {
            resultArea.value = content;
            addToHistory('魔法衣橱', userInput.substring(0, 80), content);
            const copyBtn = resultArea.parentElement?.querySelector('.copy-btn');
            if (copyBtn) copyBtn.disabled = false;
        }
        toastr.success('生成成功');
...
<label><input type="radio" name="wardrobe-mode" value="keyword" checked> 根据关键词生成</label>
<label><input type="radio" name="wardrobe-mode" value="character"> 根据人设推断</label>
<label><input type="radio" name="wardrobe-mode" value="scene"> 根据剧情分析</label>
</div>
</div>
<div class="field"><label>输入内容</label><textarea id="wardrobe-input" rows="3" placeholder="根据模式输入关键词/人设/剧情片段"></textarea></div>
...
wardrobeGen.onclick = async () => { 
    ...
    if (!wardrobeInput.value.trim()) { toastr.warning('请输入内容'); return; }
    await generateWardrobe(wardrobeInput.value.trim(), mode, wardrobeGen, wardrobeResult);
};
