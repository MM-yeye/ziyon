    ...
    if (chaptersContainer) {
        if (currentNovel.chapters.length === 0) {
            ...
            chaptersContainer.innerHTML = currentNovel.chapters.map((ch, idx) => `
                <div class="chapter-item-simple">
                    <div class="chapter-title-simple">第${idx+1}章</div>
                    <div class="chapter-content-simple">${escapeHtml(ch.content)}</div>
                </div>
            `).join('');
        }
    }
...
.dark-mode .chapter-item-simple {
    border-bottom-color: #333;
}
.dark-mode .chapter-content-simple {
    color: #e0e0e0;
}
